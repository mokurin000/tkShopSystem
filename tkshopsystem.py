...
# 全局的商品列表（模拟库存商品信息），这里先初始化为空
goods_list = []

# 在文件开头添加颜色常量
BACKGROUND_COLOR = "#E6F3FF"  # 淡蓝色背景
BUTTON_COLOR = "#4A90E2"      # 按钮颜色
BUTTON_TEXT_COLOR = "white"   # 按钮文字颜色
LABEL_COLOR = "#2C3E50"       # 标签文字颜色
FRAME_COLOR = "#FFFFFF"       # 框架背景色


# 简单的辅助函数，用于从CSV文件读取商品数据到goods_list
def load_goods_data_hn():
    global goods_list
    try:
        with open('goods.json', 'r', encoding='utf-8') as file:
            goods_list = json.load(file)
    except FileNotFoundError:
        goods_list = []  # 如果文件不存在，初始化为空列表


# 简单的辅助函数，用于将goods_list中的数据保存到CSV文件
def save_goods_data_hn():
    global goods_list
    with open('goods.json', 'w', newline='', encoding='utf-8') as file:
        json.dump(goods_list, file, ensure_ascii=False, indent=4)


# 管理员登录验证
def check_login_hn(username, password):
...
class SupermarketGUI(tk.Tk):
    ...
    def delete_goods_hn(self):
        ...
        new_goods_list = []
        for good in goods_list:
            if good['商品名称']!= del_name:
                new_goods_list.append(good)
            else:
                found = True
        if found:
    ...
    def show_view_goods_page_hn(self):
        ...
        if not goods_list:
            ...
            for row_idx, good in enumerate(goods_list, 1):
                # 商品名称
                tk.Label(
                    inner_frame,
                    text=good['商品名称'],
    ...
    def show_goods_outbound_page_hn(self):
        ...
        self.name_entry_outbound = tk.Entry(
            input_frame,
            font=('SimSun', 12),
            width=25
        )
            ...
            for good in goods_list:
                if good['商品名称'] == name:
                    ...
                    if current_quantity >= quantity:
                        good['库存数量'] = str(current_quantity - quantity)  # 转回字符串保存
                        save_goods_data_hn()
                        messagebox.showinfo("出库成功", f"商品出库成功！\n当前库存：{good['库存数量']}")
                    else:
                        messagebox.showerror("出库失败", f"库存不足！当前库存：{current_quantity}")
                    break
            else:  # 如果循环正常结束（没有找到商品）
    ...
    def show_modify_goods_page_hn(self):
        ...
        self.name_entry_modify = tk.Entry(
            input_frame,
            font=('SimSun', 12),
            width=25
        )
        ...
        self.new_quantity_entry = tk.Entry(
            input_frame,
            font=('SimSun', 12),
            width=25
        )
